...
//add comments
void displayCharacter(const Font font, char * s,
    const uint8_t y = 16,
        const int frameLength = 33,
            const uint8_t squeezeFactor = 1) {
...
//#pragma unroll_completely
        for (uint8_t line = 0; line < 16; ++line) {
            ...
            for (x = 0; x < 32; ++x) {
                ...
                gpio_put(R1, BUF[line][x][RED] * PWM_SIZE / 255 > PWM); // Output each color data to the pin
                gpio_put(R2, BUF[line + 16][x][RED] * PWM_SIZE / 255 > PWM); // Dot brightness is brighter than the current PWM counter
                gpio_put(G1, BUF[line][x][GREEN] * PWM_SIZE / 255 > PWM); // 1 when bright,
                gpio_put(G2, BUF[line + 16][x][GREEN] * PWM_SIZE / 255 > PWM); // Output 0 when it is dark
                gpio_put(B1, BUF[line][x][BLUE] * PWM_SIZE / 255 > PWM);
                gpio_put(B2, BUF[line + 16][x][BLUE] * PWM_SIZE / 255 > PWM);
                
                gpio_put(CLK, 1); // Send one clock to shift the data
                gpio_put(CLK, 0);
            }
